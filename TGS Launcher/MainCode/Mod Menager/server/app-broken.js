...
app.post('/api/payments/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
    if (!mercadopago) {
        console.error('Webhook Mercado Pago chamado, mas Mercado Pago não está configurado.');
        return res.status(500).send('Mercado Pago não configurado');
    }

    const { type, data } = req.body;
    console.log('[WEBHOOK] Evento recebido:', { type, data });
    ...
    if (type === 'payment') {
        ...
        try {
            ...
            if (paymentRow.status === 'paid') {
                console.log('Pagamento já processado:', externalRef);
                return res.status(200).send('OK');
            }
            ...
            if (payment.body.status === 'approved') {
                ...
                await connection.execute(
                    'UPDATE payments SET status = ?, paid_at = NOW(), mercadopago_payment_id = ? WHERE id = ?',
                    ['paid', paymentId, externalRef]
                );

                // Buscar usuário e mod
                ...
                const [modRows] = await connection.execute(
                    'SELECT mod_id, name, category FROM mods WHERE mod_id = ?',
                    [paymentRow.mod_id]
                );

                if (userRows.length === 0 || modRows.length === 0) {
                    console.error('Usuário ou mod não encontrado ao processar webhook.');
                    return res.status(200).send('OK');
                }
                ...
                // Definir validade da chave
                let expiryDate = null;
                let isLifetime = false;
                if (paymentRow.model === 'mensal') {
                    const d = new Date();
                    d.setDate(d.getDate() + 30);
                    expiryDate = d.toISOString().slice(0, 10);
                } else if (paymentRow.model === 'anual') {
                    const d = new Date();
                ...
                const accessKeyCode = generatePurchaseAccessKey();

                await connection.execute(
                    `INSERT INTO access_keys (user_id, code, type, status, purchase_date, expiry_date, is_lifetime, notes)
                     VALUES (?, ?, 'standard', 'active', CURDATE(), ?, ?, ?)`,
                    [
                        user.id,
                        accessKeyCode,
                        expiryDate,
                        isLifetime,
                        `Gerada por pagamento Mercado Pago (${paymentId}) para mod ${mod.name} (${paymentRow.model})`
                    ]
                );
                ...
                const mailOptions = {
                    ...
                    to: paymentRow.gift_email || user.email,
                    subject: 'Pagamento Confirmado - TGS Launcher',
                    html: `
                        <div style="max-width: 600px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif;">
                            <div style="text-align: center; margin-bottom: 30px;">
                                <h1 style="color: #6C63FF; margin: 0;">TGS Launcher</h1>
                                <p style="color: #666; margin: 5px 0;">Pagamento Confirmado</p>
                            </div>
                            
                            <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                                <h2 style="color: #333; margin-top: 0;">Olá, ${paymentRow.gift_email ? 'Amigo(a)' : user.username}!</h2>
                                <p style="color: #666; line-height: 1.6;">
                                    Seu pagamento foi confirmado com sucesso!
                                </p>
                                <div style="background: white; padding: 15px; border-radius: 5px; margin: 20px 0;">
                                    <p style="margin: 0; color: #333;"><strong>Mod:</strong> ${mod.name}</p>
                                    <p style="margin: 5px 0; color: #333;"><strong>Modelo:</strong> ${paymentRow.model}</p>
                                    <p style="margin: 5px 0; color: #333;"><strong>Valor:</strong> R$ ${paymentRow.amount_brl.toFixed(2)}</p>
                                </div>
                            </div>
                            
                            <div style="background: #e8f5e8; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
                                <h3 style="color: #2e7d32; margin-top: 0;">Sua Chave de Acesso:</h3>
                                <div style="background: white; padding: 15px; border-radius: 5px; text-align: center; margin: 20px 0;">
                                    <code style="font-size: 1.2rem; color: #6C63FF; font-weight: bold;">${accessKeyCode}</code>
                                </div>
                                <p style="color: #666; line-height: 1.6; margin: 0;">
                                    Copie esta chave e cole no aplicativo TGS Launcher para ativar o mod.
                                </p>
                            </div>
                            
                            <div style="text-align: center; color: #999; font-size: 12px; margin-top: 30px;">
                                <p>Obrigado por usar o TGS Launcher!</p>
                            </div>
                        </div>
                    `
                };
                ...
                console.log('[WEBHOOK] Email enviado com sucesso para:', paymentRow.gift_email || user.email);
            }
            ...
            const buyerMail = {
                from: process.env.EMAIL_USER,
                to: user.email,
                subject: `TGS Launcher - Chave do mod ${mod.name}`,
                html: `
                    <div style="max-width:600px;margin:0 auto;font-family:Arial, sans-serif;padding:20px;">
                        <h2 style="color:#6C63FF;">Obrigado pela sua compra!</h2>
                        <p>Olá, <strong>${user.username}</strong>!</p>
                        <p>Seu pagamento do mod <strong>${mod.name}</strong> foi confirmado pela Stripe.</p>
                        <p><strong>Modelo:</strong> ${model || 'padrão'}<br>
                           <strong>Método:</strong> ${method === 'pix' ? 'Pix' : 'Cartão'}<br>
                           <strong>Valor:</strong> ${currencyPref === 'brl' ? amountBrl.toFixed(2) + ' BRL' : amountUsd.toFixed(2) + ' USD'}</p>
                        <p>Sua chave de acesso é:</p>
                        <div style="background:#f8f9fa;padding:15px;border-radius:8px;text-align:center;margin:15px 0;">
                            <code style="font-size:1.3rem;color:#6C63FF;font-weight:bold;">${accessKeyCode}</code>
                        </div>
                        <p>Use essa chave na área <strong>Meu Perfil &gt; Minhas Chaves</strong> do launcher para ativar o mod.</p>
                        <p style="font-size:12px;color:#777;margin-top:30px;">Este é um email automático, por favor não responda.</p>
                    </div>
                `
            };
            ...
            if (giftEmail) {
                const friendMail = {
                    from: process.env.EMAIL_USER,
                    to: giftEmail,
                    subject: `Você ganhou um mod de presente - ${mod.name}`,
                    html: `
                        <div style="max-width:600px;margin:0 auto;font-family:Arial, sans-serif;padding:20px;">
                            <h2 style="color:#6C63FF;">Você recebeu um presente!</h2>
                            <p>Olá!</p>
                            <p>Você recebeu o mod <strong>${mod.name}</strong> como presente de <strong>${user.username}</strong>.</p>
                            <p>Sua chave de acesso é:</p>
                            <div style="background:#f8f9fa;padding:15px;border-radius:8px;text-align:center;margin:15px 0;">
                                <code style="font-size:1.3rem;color:#6C63FF;font-weight:bold;">${accessKeyCode}</code>
                            </div>
                            <p>Para usar, baixe o <strong>TGS Launcher</strong> e use essa chave na área de chaves.</p>
                            <p style="font-size:12px;color:#777;margin-top:30px;">Este é um email automático, por favor não responda.</p>
                        </div>
                    `
                };

                await transporter.sendMail(friendMail);
            }
...
// Gerar chave de acesso padrão para compras (sempre tipo standard no banco)
function generatePurchaseAccessKey() {
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    const year = new Date().getFullYear();
    const random = Math.random().toString(36).substring(2, 6).toUpperCase();
    return `TGS-${year}-STANDARD-${random}`;
}
...
// API de checkout de pagamento (cartão/pix) - Mercado Pago
app.post('/api/payments/checkout', authenticateToken, async (req, res) => {
    const { modId, model, method, amountUSD, currencyPreference, giftEmail, name, cpf } = req.body;

    // Sanitizar entradas básicas
    const sanitizedModId = sanitizeInput(modId);
    const sanitizedModel = sanitizeInput(model || 'standalone').toLowerCase();
    const sanitizedMethod = sanitizeInput(method || '').toLowerCase();
    const sanitizedGiftEmail = giftEmail ? sanitizeInput(giftEmail) : null;
    const sanitizedCurrencyPref = (currencyPreference === 'brl' ? 'brl' : 'usd');
    const sanitizedName = sanitizeInput(name || '');
    const sanitizedCpf = sanitizeInput(cpf || '').replace(/\D/g, '');

    if (!sanitizedModId || !sanitizedMethod) {
        return res.status(400).json({
            success: false,
            message: 'Mod e método de pagamento são obrigatórios.'
        });
    }

    if (!['cartao', 'card', 'pix'].includes(sanitizedMethod)) {
        return res.status(400).json({
            success: false,
    ...
    if (sanitizedGiftEmail && !isValidEmail(sanitizedGiftEmail)) {
        return res.status(400).json({
            success: false,
            message: 'Email do amigo inválido.'
        });
    }
    ...
    if (!mercadopago) {
        return res.status(500).json({
            success: false,
            message: 'Gateway de pagamento não configurado. Configure a MERCADOPAGO_ACCESS_TOKEN no servidor.'
        });
    }
    ...
    try {
        ...
        // Carregar mod
        const [modRows] = await connection.execute(
            'SELECT mod_id, name, category FROM mods WHERE mod_id = ? AND is_active = TRUE',
            [sanitizedModId]
        );
        ...
        // Definir valor base em USD (por enquanto confiando no amount vindo do cliente, mas garantindo mínimo > 0)
        let amountUsd = Number(amountUSD) || 0;
        if (amountUsd <= 0) {
            return res.status(400).json({
                success: false,
        ...
        // Converter para BRL para pagamento na Stripe e para registro
        const USD_TO_BRL_RATE = 5.0;
        const amountBrl = amountUsd * USD_TO_BRL_RATE;
        const amountBrlCents = Math.round(amountBrl * 100);

        // Registrar pagamento como pending
        const [paymentResult] = await connection.execute(
            `INSERT INTO payments (user_id, mod_id, model, method, amount_usd, amount_brl, currency_preference, status, gift_email, notes)
             VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
            [
                user.id,
                mod.mod_id,
                sanitizedModel,
                sanitizedMethod === 'pix' ? 'pix' : 'card',
                amountUsd,
                amountBrl,
                sanitizedCurrencyPref,
                sanitizedGiftEmail,
                `Checkout iniciado via launcher - Mod: ${mod.name} (${mod.mod_id}), modelo: ${sanitizedModel}`
            ]
        );
        ...
        const preference = {
            items: [{
                title: `Mod ${mod.name} - ${sanitizedModel}`,
                unit_price: Math.round(amountBrl * 100), // Mercado Pago usa centavos
                quantity: 1,
                currency_id: 'BRL'
            }],
            payer: {
            ...
            back_urls: {
                success: 'http://localhost:3001/open-launcher?payment=success',
                failure: 'http://localhost:3001/open-launcher?payment=cancel',
                pending: 'http://localhost:3001/open-launcher?payment=pending'
            },
            ...
            metadata: {
                ...
                giftEmail: sanitizedGiftEmail || '',
                method: sanitizedMethod === 'pix' ? 'pix' : 'card'
            }
        };
